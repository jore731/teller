...
use crate::{Provider, Result, Error, config::{PathMap, KV, ProviderInfo}}; // Import ProviderInfo
use crate::providers::ProviderKind; // Import ProviderKind for ProviderInfo
...
impl AzureKeyVault {
    pub async fn new(name: &str, vault_name: &str) -> Result<Self> {
        ...
        let credential = DefaultAzureCredentialBuilder::new().build().map_err(|e| Error::CreateProviderError(
            format!("Failed to build Azure credential for '{}': {}", name, e)
        ))?;
        let credential_arc = std::sync::Arc::new(credential);

        let client = SecretClient::new(&format!("https://{}.vault.azure.net/", vault_name), credential_arc).map_err(|e| Error::CreateProviderError(
            format!("Failed to create Azure Key Vault client for '{}': {}", name, e),
        ))?;
        Ok(Self {
            client,
            name: name.to_string(),
...
impl Provider for AzureKeyVault {
    ...
    async fn get(&self, pm: &PathMap) -> Result<Vec<KV>> {
        ...
        let secret = self.client.get(secret_name).await.map_err(|e| Error::GetError {
        ...
        Ok(vec![KV::from_value(&value, secret_name, secret_name, pm, self.kind())])
    }

    // Mark pm as unused
    async fn put(&self, _pm: &PathMap, kvs: &[KV]) -> Result<()> {
        for kv in kvs {
            self.client.set(&kv.key, &kv.value).await.map_err(|e| Error::PutError {
                path: kv.key.clone(),
    ...
    async fn del(&self, pm: &PathMap) -> Result<()> {
        ...
        self.client.delete(&pm.path).await.map_err(|e| Error::DeleteError {
            path: pm.path.clone(),
            msg: format!("Provider '{}': {}", self.name, e), // Format details into msg
        })?;
        Ok(())
...
mod tests {
    use super::*;
    use crate::config::{PathMap, KV, PathInfo, MetaInfo, Sensitivity};
    use tokio;
    use chrono::Utc;
    ...
    async fn create_provider() -> AzureKeyVault {
        AzureKeyVault::new("test-akv", &"jorgetestdevkv")
            .await
            .expect("Failed to create AzureKeyVault provider for testing. Ensure TEST_VAULT_NAME constant is set correctly.")
    }
    ...
    async fn test_azure_keyvault_put_get_delete() {
        ...
        let kv = KV {
            ...
            path: Some(PathInfo { id: pm.id.clone(), path: pm.path.clone() }),
        ...
        // PUT
        provider.put(&pm, &[kv.clone()]).await.expect("Failed to put secret");
        ...
        if let Err(Error::GetError { path, .. }) = get_result {
             ...
             println!("Verified secret deletion (get failed as expected): {}", test_key);
        } else {
    ...
    async fn test_azure_keyvault_get_nonexistent() {
        ...
        if let Err(Error::GetError { path, .. }) = get_result {
             ...
             panic!("Expected GetError for non-existent secret, but got {:?}", get_result);
        }
    }
